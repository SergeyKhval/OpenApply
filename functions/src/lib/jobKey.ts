...
function atsKey(url: URL): string | null {
...
export function jobKey(href: string): string | null {
  let original: URL;
  try {
...
export function jobKeyHash(key: string): string {
  return createHash("sha256").update(key).digest("hex");
}

// The only part of a key hash the extension sends when it looks a job up
export function jobKeyPrefix(hash: string): string {

// Monthly AI allowance rules. Mirrored in spa/src/lib/aiAllowance.ts; keep in sync.
...
/**
 * Returns the billing profile fields after spending one AI check, or null when
 * nothing is left. The monthly allowance is spent before bonus checks.
 */
export function consumeAiCheck(
  profile: AllowanceProfile | null | undefined,
  now: Date,
): { aiUsage: AiUsage; bonusChecks: number } | null {
  const state = getAllowanceState(profile, now);
  const period = usagePeriod(now);

  if (state.remaining > 0) {
    return {
      aiUsage: { period, count: state.used + 1 },
      bonusChecks: state.bonusChecks,
    };
  }
  if (state.bonusChecks > 0) {
    return {
      aiUsage: { period, count: state.used },
      bonusChecks: state.bonusChecks - 1,
    };
  }
  return null;
}
